...
// TODO: refactoring: enum for custom status codes
...
void client_worker::start()
{
    ...
    LOG(INFO) << "client worker has been started with epoll " << _epoll_d;
}
            ...
            if (event.events & EPOLLIN) {
                ...
                if (conn->state == connection_state::read_response) {
                    ...
                    if (epoll_ctl(_epoll_d, EPOLL_CTL_MOD, conn->sock_d, &event) == -1) {
                        ...
                        go_close_connection_with_error(conn);
                    }
                }
...
{
    assert(conn->state == connection_state::write_request);
    if (conn->state != connection_state::write_request) {
        LOG(WARNING) << "try handle_out but it's incorect state " << static_cast<int>(conn->state);
    }

    auto& req_reader = conn->req_reader;

    while (req_reader->has_chunks()) {
        request_reader::chunk chunk = req_reader->get_chunk();
...
void client_worker::handle_in(connection *conn) noexcept
{
    ...
    case response_state_machine::state::rejected: {
        LOG(ERROR) << "response was rejected with code " << resp_state_machine->get_rejected_code();
        go_close_connection_with_error(conn);
        break;
    }
    case response_state_machine::state::accpeted: {
        go_close_connection_with_success(conn, resp_state_machine->get_response());
        break;
    }
    }
}
...
void client_worker::go_close_connection_with_success(connection* conn,
                                                     const std::shared_ptr<response>& resp) noexcept
{
    conn->in_resp_handler(resp);
    release_connection(conn);
}

void client_worker::go_close_connection_with_error(connection* conn) noexcept
{
    conn->in_resp_handler(nullptr);
    release_connection(conn);
}

void client_worker::release_connection(connection* conn) noexcept
{
    close(conn->sock_d);
    delete conn;
}
